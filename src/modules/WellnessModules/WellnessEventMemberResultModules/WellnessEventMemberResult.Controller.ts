...
import {
  WellnessEventMemberResultPutDto,
  WellnessEventMemberResultRequestDto,
  WellnessEventMemberResultResponseDto,
  WellnessEventMemberResultQueryDto,
} from './WellnessEventMemberResult.Dto'
...
@ApiBearerAuth()
// @PublicRoute()
@Controller('wellness-event-member-result')
...
export default class WellnessEventMemberResultController {
  constructor(
    ...
    @Get()
    async index(@Query() query: WellnessEventMemberResultQueryDto) {
      const defaultLimit = query.limit || 5
      const defaultPage = query.page || 1
      const offset = (defaultPage - 1) * defaultLimit

    const data =
      await this.WellnessEventMemberResultService.getRepository().createQueryBuilder(
        'WellnessEventMemberResult',
      )
      // .leftJoinAndMapOne('WellnessEventMemberResult.programItem', 'programItem', 'programItem.program_item_id = programItems.id')
    ...
    const response = plainToInstance(WellnessEventMemberResultResponseDto, result)
    return { data: response, count: count }
  }

  @Post()
  async save(@Body() body: WellnessEventMemberResultRequestDto) {
    const data = [];
    ...
    await this.WellnessEventMemberResultService.getRepository().delete({wellness_event_member_id: body.wellness_event_member_id})
    
    let request
    data.forEach(async (el) => {
      request = await this.WellnessEventMemberResultService.getRepository().save(el)
    })

    const response = plainToInstance(WellnessEventMemberResultResponseDto, request)
  ...
  async update(
    @Param('id') id: number,
    @Body() body: WellnessEventMemberResultPutDto,
  ) {
    const query = await this.WellnessEventMemberResultService.update(id, body)
    const response = plainToInstance(WellnessEventMemberResultResponseDto, query)
    return { data: response }
  }

  @Delete(':id')
  async delete(@Param('id') id: number) {
    // await this.WellnessEventMemberResultService.update(id, { deleted_by: user.user.full_name })
    const query = await this.WellnessEventMemberResultService.delete(id)
    return { data: query }
  }

  @Get('tree')
  async getItemTree(@Query() query: WellnessEventMemberResultQueryDto) {
    const defaultLimit = query.limit || 5
    const defaultPage = query.page || 1
    const offset = (defaultPage - 1) * defaultLimit
    ...
    const resultTree = await this.WellnessEventMemberResultService.convertToTree(result)
    // console.log(tes)

    // const response = plainToInstance(WellnessEventMemberResultResponseDto, resultTree)
